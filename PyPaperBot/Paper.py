...
class Paper:
    ...
    def setBibtex(self, bibtex):
        ...
        try:
            ...
            self.jurnal = x[0]["journal"].replace(
                "\\", "") if "journal" in x[0] else None
            if self.jurnal == None:
                self.jurnal = x[0]["publisher"].replace(
                    "\\", "") if "publisher" in x[0] else None
    ...
    def generateReport(papers, path):
        ...
        for p in papers:
            ...
            # content += ("\n"+'title'+","+'scholar_link'+","+
            #             'DOI'+","+'bibtex_found'+","+ 'pdf_name'+
            #             ","+'year'+","+'scholar_page'+","+
            #             'jurnal'+","+'downloaded'+","+'dwn_from')
        file = open(path, 'a')
        file.write(content)
        file.close()
